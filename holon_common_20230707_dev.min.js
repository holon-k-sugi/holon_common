...
function getV(name, index) {
  const inputObjects = window.inputObjects;
  const radioButtons = window.radioButtons;
  console.log(radioButtons);
  console.log(window.radioButtons);
...
function onLoadDocumentEmployeesList() {
  ...
  Object.keys(employees.list).forEach(key => {
    [...Array(employees.max)].forEach((_, i) => {
      ...
      objList.forEach(obj => {
        if (i < count) setEmployeesValue(key, i, obj);
        setEventHandlerForDocumentEmployees(obj, objList);
      });
    });
  });
}
...
function setEmployeesValue(key, index, obj) {
  ...
  const value = documentEmployees.getEmployeesValue(index, keyTmp);
...
function setEventHandlerForDocumentEmployees(obj, objList) {
  getIds(obj.name, obj.page).forEach(id => {
    ...
    console.log(getSelector(obj.name, obj.page), event);
    $(getSelector(obj.name, obj.page)).on(event, (evt) => {
      console.log('hoge');
      setDocumentEmployeesContents();
      objList.forEach((_, j) => setV(objList[j].name, objList[j].page, evt.currentTarget.value));
    });
  });
}
