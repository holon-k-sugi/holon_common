...
class DocumentEmployeesContents {
  ...
  initialize() {
    try {
      ...
      console.log(getV('DOCUMENT_EMPLOYEES_CONTENTS'));
      console.log(typeof JSON.parse(getV('DOCUMENT_EMPLOYEES_CONTENTS')));
      console.log(JSON.parse(JSON.parse(getV('DOCUMENT_EMPLOYEES_CONTENTS'))));
      console.log(typeof JSON.parse(JSON.parse(getV('DOCUMENT_EMPLOYEES_CONTENTS'))));
    } catch (e) {
      this.list = [];
    }
  }
...
function onLoadDocumentEmployeesList(employees) {
  ...
  Object.keys(employees.list).forEach(key => {
    [...Array(employees.max)].forEach((_, i) => {
      ...
      objList.forEach(obj => {
        if (i < count && obj.page < inputObjects.getLengthOfPageListByName(obj.name)) setEmployeesValue(key, i, obj);
...
function setEventHandlerForDocumentEmployees(employees, obj, objList) {
  getIds(obj.name, obj.page).forEach(id => {
    ...
    $(getSelector(obj.name, obj.page)).on(event, (evt) => {
      ...
      objList.forEach((_, j) => {
        if (obj.page < inputObjects.getLengthOfPageListByName(obj.name)) setV(objList[j].name, objList[j].page, evt.currentTarget.value)
      });
    });
  });
}
...
function setDocumentEmployeesContents(employees) {
  ...
  Object.keys(employees.list).forEach(key => {
    [...Array(count)].forEach((_, i) => {
      ...
      if (obj.page < inputObjects.getLengthOfPageListByName(obj.name)) result[i][key] = getV(obj.name, obj.page);
    });
  });
