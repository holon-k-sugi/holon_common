...
function setEventHandlerForDocumentEmployees(employees, obj, objList) {
  getIds(obj.name, obj.page).forEach(id => {
    ...
    $(getSelector(obj.name, obj.page)).on(event, (evt) => {
      ...
      objList.forEach((_, j) => {
        if (obj.page < inputObjects.getLengthOfPageListByName(obj.name) && inputObjects.getIdsbyPage(objList[j].name, objList[j].page).some(id => id == evt.currentTarget.id)) setV(objList[j].name, objList[j].page, evt.currentTarget.value);
      });
    });
  });
}
