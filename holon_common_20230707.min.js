...
class RadioButton {
  ...
  getRadioButtonValue(index) {
    return Object.keys(this.reverseList).find(num => {
      return getV(this.reverseList[num], index);
    });
  }
}
...
function getN(name, i) {
  ...
  setV(name, hankakuNumber.toLocaleString('ja-JP'));
...
function onLoadDocumentEmployeesList(objectCountPerPage) {
  const employees = makeListForDocumentEmployeesList();
  // list = {full_name:(i)=>`ITEXT${3000+i}`}
  // list = {full_name:[{name:'ITEXT1000'},{name:'ITEXT1001'},{name:'ITEXT1002',page:0}]}
  const count = documentEmployees.countEmployees();
  Object.keys(employees.list).forEach(key => {
    [...Array(count)].forEach((_, i) => {
      const objList = employees.list[key](i);
      console.log(objList);
      if (!Array.isArray(objList)) {
        setEmployeesValue(key, i, objList);
      } else {
        objList.forEach(obj => {
          setEmployeesValue(key, i, obj);
        })
      }
    });
  });
...
function setEmployeesValue(key, index, obj) {
  const value = documentEmployees.getEmployeesValue(index, key);
  if (!!value) setV(obj.name, obj.page, splitEmployeesValue(key, value));
}

function setEventHandlerForDocumentEmployees(obj, objList) {
  getIds(obj.name, obj.page).forEach(id => {
    ...
    $(getSelector(obj.name, obj.page)).on(event, (evt) => {
      setDocumentEmployeesContents();
      obj.forEach((_, j) => setV(objList[j].name, objList[j].page, evt.currentTarget.value));
    });
  });
}
...
function splitEmployeesValue(key, value) {
  ...
  if (keyName == 'birthday' || keyName == 'hire_date') {
    return toWareki(value)[keyName];
  }
  if (keyName == 'employment_insurance_number') {
    return value.split('-')[keyName];
  }
  return value;
}

function setDocumentEmployeesContents() {
  const employees = makeListForDocumentEmployeesList();
  ...
  Object.keys(employees.list).forEach(key => {
    [...Array(employees.max)].forEach((_, i) => {
      ...
      result[i][key] = getV(obj.name, obj.page);
    });
  });
...
if (window.allObj != undefined) {
  ...
  console.log('ライブラリ名：holon_common_20230707');
...
function makeListForDocumentEmployeesList() {
  const ret = { list: {}, max: {} };
  ret.max = 5;
  const objectCount = 7; //1ページあたりのオブジェクト数
  const makeMyArray = (prefix, first, deference) => makeArray(objectCount, prefix, first, deference);
  const generateName = (prefix, first, deference) => (p, f, d) => `${p}${f + i % objectCount * d}`;
  const generatelistObj = (prefix, first, deference) => { return (i) => { return { name: generateName(prefix, first, deference), page: Math.floor(i / objectCount) }; }; }
  ret.list.full_name = (i) => { return { name: `ITEXT${3002 + i % objectCount * 10}`, page: Math.floor(i / objectCount) }; };
  // list.first_name = [];
  // list.last_name = [];
  // list.birthday = [...Array(objectCountPerPage)].map((_, i) => makeArray(3, 'ITEXT', 1000 + i * 7, 1));
  // list.hire_date = [];
  // list.employment_insurance_number = makeMyArray('ITEXT', 3006, 10);
  // list.date1 = makeMyArray('ITEXT', 3002, 10);
  // list.date1 = makeMyArray('ITEXT', 3003, 10);
  // list.date2 = makeMyArray('ITEXT', 3004, 10);
  return ret;
}
